...
/*   Updated: 2022/04/13 22:20:41 by pmolnar       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
// sort stack by radix
void	sort_stack(t_node **stk_a, t_node **stk_b)
{
	long int	bit_mask;
	t_node			*tmp;
	size_t			counter;
	...
	tmp = *stk_a;
	counter = 0;
	while (!is_stack_sorted(*stk_a, get_sllist_size(*stk_a), ASCENDING))
	{
		printf("check %zu bit from left:\n", counter + 1);
		while (!is_stack_separated(*stk_a, counter, 1))
		{
